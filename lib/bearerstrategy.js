...
Strategy.prototype.jwtVerify = function jwtVerifyFunc(req, token, metadata, optionsToValidate, done) {
  ...
  // console.log('*** START jwtVerify ***', {token, metadata: !!metadata, optionsToValidate: !!optionsToValidate})
  ...
  if (decoded === null) {
    console.log(' -------------------------------------------------------------- ')
    console.log(' Passport-azure-ad: Authentication FAILED ')
    console.log(" ERROR: In Strategy.prototype.jwtVerify: Invalid JWT token.", { token, decoded });
    console.log(' -------------------------------------------------------------- ')
    return done(null, false, "In Strategy.prototype.jwtVerify: Invalid JWT token.");
  }

  // if (self._options.loggingNoPII)
  //   console.log("In Strategy.prototype.jwtVerify: token is decoded");
  // else
  //   console.log("In Strategy.prototype.jwtVerify: token decoded: ", decoded);
  ...
  try {
    if (decoded.header.x5t) {
      ...
      // console.log("In Strategy.prototype.jwtVerify: We did not receive a token we know how to validate");
  ...
  // if (self._options.loggingNoPII)
  //   console.log("PEMkey generated");
  // else
  //   console.log("PEMkey generated: " + PEMkey);

  jwt.verify(token, PEMkey, optionsToValidate, (err, verifiedToken) => {

    // console.log('jwt.verify callback: ', { err, verifiedToken: !!verifiedToken });

    if (err) {
      if (err.message && !self._options.loggingNoPII) {
        console.log('ERROR 1: In Strategy.prototype.jwtVerify: cannot verify token: ', err.message);
        return self.failWithLog(err.message);
      }
      else {
        console.log("ERROR 2: In Strategy.prototype.jwtVerify: cannot verify token", { err }); // HERE
    ...
    if (optionsToValidate.scope) {
      if (!verifiedToken.scp) {
        // console.log("In Strategy.prototype.jwtVerify: scope is not found in token");
      ...
      if (!hasValidScopeInToken) {
        if (self._options.loggingNoPII) {
          // console.log("In Strategy.prototype.jwtVerify: none of the scopes in token is accepted");
          return self.failWithLog("In Strategy.prototype.jwtVerify: none of the scopes in token is accepted");
        }
        else {
          // console.log(`In Strategy.prototype.jwtVerify: none of the scopes '${verifiedToken.scp}' in token is accepted`);
    ...
    // if (self._options.loggingNoPII)
    //   console.log("In Strategy.prototype.jwtVerify: token is verified");
    // else
    //   console.log("In Strategy.prototype.jwtVerify: VerifiedToken: ", verifiedToken);

    if (self._options.passReqToCallback) {
      // console.log("In Strategy.prototype.jwtVerify: We did pass Req back to Callback");
      // // const result = self._verify(req, verifiedToken, done);
      // console.log(' ----------------------------------------- ')
      // console.log('Calling self._verify(req, verifiedToken, done) NOW')
      // // return result //self._verify(req, verifiedToken, done);

      return self._verify(req, verifiedToken, done);
    } else {
      // console.log("In Strategy.prototype.jwtVerify: We did not pass Req back to Callback");
      // // const result = self._verify(verifiedToken, done);
      // console.log('Calling self._verify(verifiedToken, done)', {result})
      // // return result // self._verify(verifiedToken, done);
...
Strategy.prototype.authenticate = function authenticateStrategy(req, options) {
  ...
  // console.log(' Params: ', { params, tenantIdOrName })
  ...
  async.waterfall([
    ...
    (next) => {
      ...
      if (!self._options._isCommonEndpoint && tenantIdOrName) {
        // if (self._options.loggingNoPII)
        //   console.log("identityMetadata is tenant-specific, so we ignore the provided tenantIdOrName");
        // else
        //   console.log(`identityMetadata is tenant-specific, so we ignore the tenantIdOrName '${tenantIdOrName}'`);
        tenantIdOrName = null;
      }
      ...
      if (self._options._isCommonEndpoint && tenantIdOrName) {
        ...
        // if (self._options.loggingNoPII)
        //   console.log("We are replacing 'common' with the provided tenantIdOrName");
        // else
        //   console.log(`we are replacing 'common' with the tenantIdOrName ${tenantIdOrName}`);
      }
      ...
      if (self._options._isCommonEndpoint && self._options.validateIssuer &&
        (!self._options.issuer && !tenantIdOrName)) {
          // console.log("In passport.authenticate: issuer or tenantIdOrName must be provided in order to validate issuer on common endpoint");
      ...
      if (self._options.isB2C && self._options._isCommonEndpoint && !tenantIdOrName) {
        // console.log("In passport.authenticate: we are using common endpoint for B2C but tenantIdOrName is not provided");
      ...
      // if (!self._options.loggingNoPII)
      //   console.log(`In Strategy.prototype.authenticate: ${JSON.stringify(params)}`);
    ...
    (metadata, next) => {
      ...
      // if (self._options.loggingNoPII)
      //   console.log("In Strategy.prototype.authenticate: received metadata");
      // else
      //   console.log(`In Strategy.prototype.authenticate: received metadata: ${JSON.stringify(metadata)}`);
      ...
      // if (self._options.loggingNoPII)
      //   console.log("In Strategy.prototype.authenticate: we will validate the options");
      // else
      //   console.log(`In Strategy.prototype.authenticate: we will validate the following options: ${JSON.stringify(optionsToValidate)}`);
    ...
    (next) => { // eslint-disable-line no-unused-vars -- Next used in async.waterfall
      ...
      if (req.query && req.query.access_token) {
        // console.log("In Strategy.prototype.authenticate: access_token should be passed in request header or body. query is unsupported");
      ...
      if (req.headers && req.headers.authorization) {
        ...
        if (auth_components.length === 2 && auth_components[0].toLowerCase() === "bearer") {
            ...
            if (token !== "") { // eslint-disable-line security/detect-possible-timing-attacks -- Timing for comparison to empty string should be the same every time
              // if (self._options.loggingNoPII)
              //   console.log("In Strategy.prototype.authenticate: access_token is received from request header");
              // else
              //   console.log(`In Strategy.prototype.authenticate: received access_token from request header: ${token}`);
            }
            else {
              // console.log("In Strategy.prototype.authenticate: missing access_token in the header");
      ...
      if (req.body && req.body.access_token) {
        if (token) {
          // console.log("In Strategy.prototype.authenticate: access_token cannot be passed in both request header and body");
        ...
        if (token) {
          // if (self._options.loggingNoPII)
          //   console.log("In Strategy.prototype.authenticate: access_token is received from request body");
          // else
          //   console.log(`In Strategy.prototype.authenticate: received access_token from request body: ${token}`);
        }
      }
      ...
      function verified(err, user, info) {
        ...
        if (!user) {
          ...
          if (self._options.loggingNoPII) {
            ...
            return self.failWithLog('err_message: ', err_message);
          }
        }

        // console.log('return self.success', { user: !!user, info: !!info })
        return self.success(user, info);
      }

      // const result = self.jwtVerify(req, token, params.metadata, optionsToValidate, verified)
      // console.log('return self.jwtVerify', { has_result: !!result, result, verified })
      // console.log('Calling self.jwtVerify NOW')

      // return result // self.jwtVerify(req, token, params.metadata, optionsToValidate, verified);
    ...
    (waterfallError) => { // This function gets called after the three tasks have called their 'task callbacks'
      if (waterfallError) {
        // console.log('waterfallError: ', waterfallError);
